...
sitemap ends is ``/sitemap.{_format}``. If you configure several sitemaps (see
...
Json sitemaps are generated by serializing the information to Json. You can
change the formatting and add other formats by writing twig templates. The
template is passed a list of ``UrlInformation`` objects in the parameter
``urls``. A good starting point for a custom template is the default template
provided by the bundle for that format.

The templates are specified in the ``defaults`` section or for a specific sitemap.
